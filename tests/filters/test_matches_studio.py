...
def test_matches_studio_with_name():
    ...
    run_context = create_stash_context(scene=create_scene(studio=studio), studios=[studio])

    filter_settings = MatchesStudioFilterSettings(name="My Studio")
    assert matches_studio(run_context, filter_settings)
...
def test_matches_studio_with_no_match():
    ...
    assert matches_studio(run_context, filter_settings) is False


def test_scene_with_no_studio():
    scene = create_scene(studio=None)

    run_context = create_stash_context(scene=scene, studios=[create_studio()])
...
def test_with_no_studios():
    ...
    run_context = create_stash_context(scene=scene, studios=[])
